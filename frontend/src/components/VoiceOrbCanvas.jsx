 ...
 * - audioLevel: number (0 to 1) representing current live mic or TTS volume
 */
    ...
    const render = () => {
      ...
      layers.forEach((layer, idx) => {
        ...
        for (let i = 0; i < numPoints; i++) {
          ...
          const noise = Math.sin(angle * 3 + t * 2 + layer.phase) * (10 + amp * 25);
      ...
      if (state !== 'idle') {
        ...
        for (let i = 0; i < numParticles; i++) {
          ...
          const pDist = baseRadius * 1.35 + Math.sin(t * 3 + i) * 8 + amp * 15;
          const px = centerX + Math.cos(pAngle) * pDist;
          const py = centerY + Math.sin(pAngle) * pDist;
          const pSize = 2 + Math.sin(t * 4 + i) * 1.5;
